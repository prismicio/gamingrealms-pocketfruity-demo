...
app.use('*', (req, res, next) => {
  req.prismic.api.getSingle("side_menu")
  .then((sidemenu) => {
    res.locals.sidemenu = sidemenu;
    next();
  })
  .catch((err) => {
    next(`Error getting side menu from prismic: ${error.message}`);
  });
});

//redirect / to default language from i18n.json
app.get('/', (req, res, next) => {
  res.redirect('/slots/colossal-bar-x');
});

// Route for pages
app.get('/slots/:uid', (req, res, next) => {
  const uid = req.params.uid;
  
  req.prismic.api.getByUID("game_info_page", uid)
  .then((page) => {
    if(!page) res.status(404).send('page not found');
    else res.render('game', { page });
  })
  .catch((error) => {
